// scripts/prices/scrapers/latiendadelpadel.js
// WooCommerce — API REST pública (wc/store/v1), no hace falta cheerio/HTML
// https://latiendadelpadel.com/wp-json/wc/store/v1/products?category=palas-de-padel

const { detectarRebajasYCodigoViaHtml } = require('./_discount-utils.js')

const SOURCE_KEY = 'latiendadelpadel'
const BASE_URL   = 'https://latiendadelpadel.com'
...
// ej. "9795" = 97,95€. minor_unit indica los decimales (normalmente 2).
...
async function scrape() {
  console.log('[latiendadelpadel] Iniciando scraper (WooCommerce Store API)…')
  ...
  while (page <= totalPages) {
    ...
    try {
      ...
      console.error(`[latiendadelpadel] Error página ${page}:`, e.message)
      break
    }
    ...
    console.log(`[latiendadelpadel] página ${page}/${totalPages} → ${data.length} productos`)

    page++
    if (page <= totalPages) await sleep(DELAY_MS)
  }

  console.log(`[latiendadelpadel] Total palas: ${allProducts.length}`)
  ...
  if (codigoDescuento) {
    console.log(`[latiendadelpadel] codigo detectado: ${codigoDescuento.codigo} (-${codigoDescuento.descuento_pct}%)`)
  }
  if (rebajasUrls.length > 0) {
    console.log(`[latiendadelpadel] sección(es) de rebajas detectada(s) (no scrapeada automáticamente): ${rebajasUrls.join(', ')}`)
  }
