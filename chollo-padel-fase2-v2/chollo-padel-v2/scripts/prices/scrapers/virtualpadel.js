...
async function scrape() {
  ...
  // Para productos sin imagen del listing, extraer og:image de la ficha (WooCommerce + Elementor lazy-load).
  const sinImagen = allProducts.filter(p => !p.image)
  if (sinImagen.length > 0) {
    console.log(`[virtualpadel] Completando imagen de ficha para ${sinImagen.length} productos sin imagen\u2026`)
    for (const p of sinImagen) {
      try {
        ...
        console.error(`[virtualpadel] No se pudo obtener imagen de ${p.url}:`, e.message)
      }
      await sleep(DELAY_MS)
    }
  }
